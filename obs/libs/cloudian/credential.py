...
def get_credential(data=None, json=None, method="GET"):
    ...
    credentials = requestors.request(url=url, data=data, json=json, method=method)
...
def create(data=None, json=None, method="PUT"):
    user_credentials = requestors.request(
        url=user.base_url + "/credentials", data=data, json=json, method=method
    )
...
def update(data=None, json=None, method="POST"):
    user_credentials = requestors.request(
        url=user.base_url + "/credentials/status", data=data, json=json, method=method
    )
